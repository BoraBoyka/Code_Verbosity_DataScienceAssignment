...
namespace ACDMAutomation.Shared.PageObjects
{
    public class CommonOperationUtils
    {
        ...
        /// <summary>
        /// Method for Click Operation without generating dynamic xpath
        /// </summary>
        /// <param name="objDriver">Browser Object<param>
        /// <param name="pageName">XML FileName Where Element To Be Checked</param>
        /// <param name="elementName">Element Xpath to Be Searched</param>
        /// <returns>Element Is Clickable Or Not</returns>
        public bool ClickElementWithXpath(IWebDriver objDriver, string xpath)
        {
            try
            {
                objDriver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(10);
                IWebElement clickElement = objDriver.FindElement(By.XPath(xpath));
                clickElement.Click();
                return true;
            }
            catch
            {
                Assert.IsFalse(true, $"Failed_To_Click_Element");
                return false;
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// Scroll horizontally on a web page based on element Xpath
        /// </summary>
        /// <param name="Xpath"></param>
        public void scrollUntilElementIsFound(string Xpath)
        {
            IJavaScriptExecutor js = (IJavaScriptExecutor)_driver;        		
            WebElement Element = (WebElement)_driver.FindElement(By.XPath(Xpath));		
        ...
        /// <summary>
        /// Method to fetch text from an element that is hidden on the screen
        /// </summary>
        /// <param name="objDriver">Browser Object</param>
        /// <param name="elementXPath">Element XPath</param>
        /// <returns>Fetched text from an element</returns>
        public static string GetTextFromHiddenElementWithoutXpath(IWebDriver objDriver, string elementXPath)
        {
            try
            {
                IWebElement hiddenElement = objDriver.FindElement(By.XPath(elementXPath));
                String hiddenContent = hiddenElement.GetAttribute("textContent").Trim();               
                return hiddenContent.ToString();
            }
            catch (Exception e)
            {
                Assert.IsFalse(true, $"Failed_To_Fetch_Text_Hidden_Element={e.Message}");
                return null;
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// Method for Disabled check Operation 
        /// </summary>
        /// <param name="objDriver">Browser Object<param>
        /// <param name="pageName">XML FileName Where Element To Be Checked</param>
        /// <param name="elementName">Element Xpath to Be Searched</param>
        /// <returns>Element Is Enabled Or Not</returns>
        public static bool IsDisabled(IWebDriver objDriver, string pageName, string elementName)
        {
            try
            {
                objDriver.Manage().Timeouts().ImplicitWait = TimeSpan.FromSeconds(10);
                IWebElement elementXpath = objDriver.FindElement(By.XPath(XMLReaderUtil(pageName, elementName)));
                Assert.IsTrue(elementXpath.GetAttribute("aria-disabled").Equals("true"), "Element is Read-only");
                return true;
            }
            catch
            {
                Assert.IsFalse(true, "Element to be verified is Enabled on the screen");
                return false;
            }
        }
    }
